	...
	PointCloud::Ptr pointcloud( new PointCloud );
  	...
  	while(1)
  	{
		...
		for (int i = 0; i < matches.size(); ++i)
		{
			...
			i_kp.push_back(matches[0].queryIdx);
            j_kp.push_back(matches[0].trainIdx);
		}


		findFundamentalMat(out_p1, out_p2, FM_RANSAC, 3.0, 0.99, mask2);


		Mat canvas = prevImage.clone();
        canvas.push_back(currImage.clone());

        for (size_t k=0; k < mask2.size(); k++) 
        {
            if (mask2[k]) 
            {
                j_kp[k];
                ...
                line(canvas, out_p1[k], out_p2[k] + Point2f(0, currImage.rows), Scalar(0, 0, 255), 2);
            }
        }

        //imshow("img", canvas);
        //imwrite("img.jpg", canvas);

		int good_matches = sum(mask2)[0];
		cout << "Feature matching " << good_matches << "/" << matches.size() << endl;
